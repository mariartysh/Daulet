// Ядро охоты: обход расписания, бронирование, отмена, напоминания, сводка.
...
const COMBO_CAP = Number(process.env.COMBO_CAP || 8); // запросов расписания за один проход — щадим сайт
...
async function ensureStaff(s) {
  ...
  for (const b of bases) {
    ...
    if (j.success && Array.isArray(j.data)) {
      if (s.apiBase !== b) { s.apiBase = b; log(s, `API подключено: ${b}`); }
      break;
    }
    lastErr = `HTTP ${j._status} ${JSON.stringify(j.meta || j.raw || '').slice(0, 100)}`;
    j = null;
  }
  if (!j) throw new Error(`book_staff не ответил ни на одной базе (${lastErr}). Проверьте ALTEGIO_AUTH — возможно, токен устарел, снимите заново из DevTools`);
...
async function ensureServices(s, staffId) {
  ...
  const list = (Array.isArray(raw) ? raw : []).map(x => ({
    ...
    len: Number(x.session_length || x.seance_length || 0) / 60 || null, // минуты
...
function courtTitle(meta, name) {
  if (meta.court != null) return `Корт ${meta.court}${meta.indoor === null ? '' : meta.indoor ? ' (Крытый)' : ' (Уличный)'}`;
  return name;
}
...
async function sweep(s) {
  ...
  if (remaining() <= 0) return finishTask(s, 'цель уже достигнута');

  let staff;
  try { staff = await ensureStaff(s); }
  catch (e) { s.stats.errors++; log(s, 'Ошибка API: ' + e.message, 1); await notifyError(s, e.message); return { error: e.message }; }

  const courts = staff.filter(m => L.courtOk(t, m));
  if (!courts.length) { log(s, 'Фильтр пуст: нет подходящих кортов', 1); return { error: 'no courts' }; }
  const dates = L.taskDates(t, now);
  if (!dates.length) { log(s, 'Даты задания в прошлом — остановите или обновите задание', 1); return { error: 'no dates' }; }

  // комбинации дата×корт, ротация чтобы за минуту покрыть всё
  const combos = [];
  ...
  for (const f of foundNew) {
    ...
    else await notifySlot(s, f);
  }
  if (t.mode === 'confirm' && foundNew.length) log(s, `Найдено новых слотов: ${foundNew.length} — отправлено в Telegram`);
  if (!foundNew.length) log(s, `Проверка №${s.stats.checks}: новых слотов нет`);
...
async function doBook(s, staffId, serviceId, startMs) {
  ...
  const iso = new Date(startMs).toISOString().replace(/\.\d+Z/, '+00:00'); // Altegio принимает ISO с TZ
  const j = await alt.book({
    ...
    comment: 'Бронь через автобот'
  }, s.apiBase);
  if (!j.success) {
    const code = (j.meta && j.meta.code) || (Array.isArray(j.errors) && j.errors[0] && j.errors[0].code);
    if (j._status === 422 && (code === 433 || code === 437)) {
      log(s, `Слот уже заняли (${courtTitle(staff, staff.name)} ${L.fmt(startMs)}) — продолжаю охоту`);
      return { ok: false, why: 'taken' };
    }
    s.stats.errors++;
    const why = JSON.stringify(j.meta || j.data || j.raw || '').slice(0, 160);
    log(s, `Бронь не прошла (${courtTitle(staff, staff.name)} ${L.fmt(startMs)}): ${why}`, 1);
    await notifyError(s, `Бронь не прошла: ${courtTitle(staff, staff.name)} ${L.fmt(startMs)}\n${why}`);
    return { ok: false, why };
  }
  ...
  s.bookings.push(b);
  s.stats.booked++;
  log(s, `Бронь создана: ${courtTitle(staff, staff.name)} ${L.fmt(startMs)} ✅`);
  const done = t.needed - activeBookings(s).length <= 0;
  if (done) t.active = false;
  const d = L.deadlines(startMs);
  await ownerSend(s,
    `🤖 ✅ <b>Автобронь сработала!</b>\n` +
    `${courtTitle(staff, staff.name)}, ${L.fmt(startMs)}${L.midnightNote(startMs)}\n` +
    (b.service ? `Услуга: ${b.service}\n` : '') +
    (b.price ? `Цена: ${b.price} ₸\n` : '') +
    `На имя: ${s.profile.name || '—'}\n` +
    `Отмена: онлайн до ${L.hm(d.online)} · ресепшн до ${L.hm(d.phone)}\n` +
    (done ? `\nЗадание охоты выключено (${t.needed}/${t.needed}). Новое — через /auto\n` : '') +
    `\n📍 Daulet Tennis Academy, ул. Кордай, 6. Оплата на месте.`,
...
async function doCancel(s, bookingId) {
  ...
  if (!b) return { ok: false, why: 'Бронь не найдена или уже отменена' };
  const d = L.deadlines(b.start);
  if (Date.now() > d.online) {
    return { ok: false, why: Date.now() > d.phone
      ? 'Отмена закрыта (меньше 3 часов до начала)'
      : `Онлайн-отмена закрыта. До ${L.hm(d.phone)} можно отменить звонком на ресепшн.` };
  }
  if (b.recordId && b.hash) {
    const j = await alt.cancel(b.recordId, b.hash, s.apiBase);
    if (!j.success && j._status !== 204 && j._status !== 200) {
      log(s, `Отмена через API не прошла (HTTP ${j._status}) — отмените на сайте/по телефону`, 1);
      return { ok: false, why: `Сайт отклонил отмену (HTTP ${j._status}). Отмените вручную на сайте или по телефону.` };
    }
  }
  b.cancelled = true;
  log(s, `Бронь отменена: ${courtTitle(b, b.name)} ${L.fmt(b.start)}`);
  return { ok: true, b };
}

// ---- напоминания о дедлайне онлайн-отмены ----
async function reminders(s) {
  ...
  for (const b of activeBookings(s)) {
    ...
    if (!s.reminded[b.id] && now >= d.online - 65 * 60e3 && now < d.online) {
      s.reminded[b.id] = 1;
      await ownerSend(s,
        `⏳ <b>Через час закроется онлайн-отмена</b>\n${courtTitle(b, b.name)}, ${L.fmt(b.start)}${L.midnightNote(b.start)}\n` +
        `Онлайн-отмена до ${L.hm(d.online)}, ресепшн до ${L.hm(d.phone)}.`,
        { reply_markup: { inline_keyboard: [[{ text: '↩️ Отменить сейчас', callback_data: `c|${b.id}` }]] } });
    }
  }
}

// ---- уведомления ----
async function ownerSend(s, text, extra) {
  if (!s.ownerChat) return;
  await tg.send(s.ownerChat, text, extra || {});
}
...
async function notifySlot(s, f) {
  ...
  await ownerSend(s,
    `🎾 <b>Новые слоты!</b>\n\n${title}\n${L.dm(f.startMs)} (${L.wd(f.startMs)}): ${L.hm(f.startMs)}${L.midnightNote(f.startMs)}\n\n👇 Нажми, чтобы забронировать`,
    { reply_markup: { inline_keyboard: [[{ text: `Забронировать ${L.hm(f.startMs)}`, callback_data: `b|${f.staff.id}|${f.svc ? f.svc.id : 0}|${ts}` }]] } });
}

async function notifyError(s, msg) {
  await ownerSend(s, `⚠️ <b>Ошибка</b>\n${String(msg).slice(0, 500)}`);
}

async function finishTask(s, why) {
  if (s.task.active) {
    s.task.active = false;
    log(s, `Задание выполнено — охота остановлена 🏁${why ? ' (' + why + ')' : ''}`);
  }
  return { done: true };
}

// ---- статус для /status и панели ----
function statusText(s) {
  const t = s.task;
  const act = activeBookings(s).sort((a, b) => a.start - b.start);
  let out = `${t.active ? '🟢 Охота идёт' : '⚪ Задание выключено'} · цель ${t.needed}, есть ${act.length}\n`;
  out += `Режим: ${t.mode === 'auto' ? 'автобронь' : 'по кнопке'} · ${t.timeFrom}–${t.timeTo} · ${Math.round(t.dur / 60)} ч\n`;
  const DN = ['сегодня', 'завтра', 'послезавтра'];
  const days = (t.dayOffsets && t.dayOffsets.length ? t.dayOffsets : [0, 1, 2]).map(o => DN[o] || o).join(', ');
  const type = t.type === 'indoor' ? 'крытые' : t.type === 'outdoor' ? 'уличные' : 'любой тип';
  out += `Дни: ${days} · ${type} · корт: ${t.courts && t.courts.length ? '№' + t.courts.join(', №') : 'любой'}\n`;
  out += `Проверок: ${s.stats.checks} · найдено: ${s.stats.found} · ошибок: ${s.stats.errors}\n`;
  if (act.length) {
    out += '\n<b>Брони:</b>\n';
    for (const b of act) {
      const d = L.deadlines(b.start);
      out += `• ${courtTitle(b, b.name)} — ${L.fmt(b.start)}${L.midnightNote(b.start)}\n  отмена: онлайн до ${L.hm(d.online)} / ресепшн до ${L.hm(d.phone)}\n`;
    }
  }
  return out;
}

module.exports = { sweep, doBook, doCancel, reminders, statusText, activeBookings, ensureStaff, courtTitle, ownerSend };
