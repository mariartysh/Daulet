// Ядро охоты: поиск целей (корт = сотрудник ИЛИ услуга), обход расписания, бронь, отмена.
...
// ---------- обнаружение кортов ----------
// Altegio настраивают двумя способами: корт = «сотрудник» (мастер) или корт = «услуга».
// Раньше бот понимал только первый — из-за этого и «ничего не находится».
async function ensureTargets(s) {
  ...
  if (!staffRaw) throw new Error(`сайт кортов не отвечает (${lastErr}). Скорее всего устарел ключ ALTEGIO_AUTH — снимите его заново из DevTools`);

  // ВАЖНО: bookable не фильтруем — без выбранной услуги Altegio часто отдаёт bookable:false
  const staff = staffRaw.map(x => ({ id: Number(x.id), name: x.name || '', ...L.parseCourt(x.name) }));

  const list = [];
  const staffWithCourt = staff.filter(x => x.court != null);

  if (staffWithCourt.length) {                      // модель 1: корт = сотрудник
    for (const st of staffWithCourt) {
      const svcs = await servicesFor(s, st.id);
      list.push({ staffId: st.id, court: st.court, indoor: st.indoor, name: st.name, services: svcs });
    }
  } else {                                          // модель 2: корт = услуга
    const all = await servicesFor(s, 0);
    const byCourt = all.filter(x => x.court != null);
    const pool = byCourt.length ? byCourt : all;
    for (const sv of pool) {
      const host = (sv.staff && sv.staff.length ? sv.staff : staff.map(x => x.id));
      for (const sid of host.slice(0, 3)) {
        const st = staff.find(x => x.id === Number(sid)) || staff[0] || { id: 0, name: '' };
        list.push({
          staffId: Number(sid) || st.id || 0,
          court: sv.court != null ? sv.court : st.court,
          indoor: sv.indoor != null ? sv.indoor : st.indoor,
          name: sv.title || st.name, services: [sv]
        });
      }
    }
  }
  if (!list.length) throw new Error('сайт отвечает, но кортов в ответе нет — проверьте COMPANY_ID и ключ');
  s.targets = { ts: Date.now(), list };
  log(s, `Нашёл кортов на сайте: ${list.length}`);
  return list;
}
...
async function servicesFor(s, staffId) {
  ...
  const list = (Array.isArray(raw) ? raw : []).map(x => ({
    ...
    staff: x.staff ? x.staff.map(st => Number(st.id || st)) : null,
    ...L.parseCourt(x.title)
  }));
  s.svcCache.byStaff[key] = list;
  return list;
}

// Услуга под нужную длительность; если точной нет — берём любую (сайт всё равно покажет доступное время)
...
async function sweep(s) {
  ...
  if (!courts.length) {
    log(s, `Под фильтр не подходит ни один корт (всего на сайте ${targets.length}) — ослабьте фильтр`, 1);
    return { error: 'no courts' };
  }
  ...
  for (const { d, c } of batch) {
    ...
    for (const slot of j.data) {
      const startMs = slot.datetime ? Date.parse(slot.datetime)
        : Date.parse(`${d.iso}T${(slot.time || '').slice(0, 5)}:00+0${Number(process.env.TZ_OFFSET) || 5}:00`);
  ...
  if (!apiOk && apiFail) {
    s.stats.errors++;
    log(s, `Расписание не отдалось ни по одному корту (${apiFail} попыток) — вероятно, устарел ключ доступа`, 1);
  }
...
async function offerSlot(s, f) {
  ...
  const o = {
    ...
    start: f.startMs, dur: s.task.dur, price: f.svc && f.svc.price
  };
...
async function doBook(s, staffId, serviceId, startMs) {
  ...
  const j = await alt.book({
    ...
    staffId: Number(staffId), serviceId: (serviceId ? Number(serviceId) : (svc ? svc.id : null)), datetime: iso,
  ...
  if (!j.success) {
    ...
    if (j._status === 422 && (code === 433 || code === 437)) {
      log(s, `Не успел — ${title} ${L.whenText(startMs, t.dur)} уже увели`);
      return { ok: false, why: 'Не успели — слот уже заняли' };
    }
    s.stats.errors++;
...
async function notifyError(s, msg) {
  const now = Date.now();
  if (s.lastErrPing && now - s.lastErrPing < 30 * 60e3) return; // не спамим одинаковыми сбоями
  s.lastErrPing = now;
...
function statusText(s) {
  ...
  out += `🗓 ${days} · ${L.hourLabel(parseInt(t.timeFrom))}–${L.hourLabel(parseInt(t.timeTo))} · ${Math.round(t.dur / 60)} ч\n`;
  ...
  if (s.targets && s.targets.list) out += `\n🏟 Кортов на сайте видно: ${s.targets.list.length}`;
  if (act.length) {
    out += '\n\n<b>Брони:</b>';
    for (const b of act) {
      const d = L.deadlines(b.start);
