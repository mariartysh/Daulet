// Пингуется cron-job.org раз в минуту: /api/tick?key=TICK_KEY (2 прохода с паузой).
// Vercel cron раз в день дергает /api/tick?job=summary (сводка).
// Состояние перечитывается перед каждым проходом, чтобы не затирать правки из панели/бота.
...
module.exports = async (req, res) => {
  ...
  if (q.job === 'summary') {
    ...
    await hunt.sendAll(s, `📊 <b>Сводка за сутки</b>\n${hunt.statusText(s)}`);
    await store.save(s);
